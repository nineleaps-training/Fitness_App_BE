...
public class UserOrderService implements UserOrderDAO {
    ...
    public UserOrder updateOrder(Map<String, String> data) {
        ...
        if (optional.isPresent()) {
            ...
            order.setBooked(CURRENT_STRING);
            order.setDate(date);
            order.setTime(time);
            ...
            if (order.getSubscription().equals(MONTHLY)) {
                booked = 30;
            } else if (order.getSubscription().equals(QUARTERLY)) {
                booked = 90;
    ...
    public List<UserOrder> pendingListOrder(String email) {
        ...
        orders = orders.stream().filter(o -> o.getStatus().equals(CREATED)).collect(Collectors.toList()); // pending
                                                                                                          // order
                                                                                                          // list of
                                                                                                          // user
    ...
    public List<UserOrder> orderListOrder(String email) {
        ...
        List<UserOrder> orders = userOrderRepo.findByEmail(email);
        orders = orders.stream().filter(o -> o.getStatus().equals(COMPLETED)).collect(Collectors.toList()); // Fetching
                                                                                                            // order
                                                                                                            // details
                                                                                                            // which
                                                                                                            // are
    ...
    public List<GymRepresentModel> bookedGym(String email) {
        ...
        for (UserOrder order : orders) {
            if (order.getBooked().equals(CURRENT_STRING)) {
                gyms.add(gymService.getGymByGymId(order.getGym()));
            }
        }
        orders = orders.stream().filter(o -> o.getBooked().equals(EXPIRED)).collect(Collectors.toList()); // Fetching
                                                                                                          // all gyms
                                                                                                          // which are
                                                                                                          // booked
        for (UserOrder order : orders) {
    ...
    public Boolean canOrder(String email) {
        ...
        List<UserOrder> orders = userOrderRepo.findByEmail(email);
        if (orders == null) {
            log.warn(ORDER_LIST_EMPTY);
            return true;
        }
        orders = orders.stream().filter(o -> o.getBooked().equals(CURRENT_STRING)).collect(Collectors.toList());
